...
public class GenerateCloudInitSingleNode implements StepBody {

    private static final Logger LOGGER = Logger.getLogger(GenerateCloudInitSingleNode.class);

    @Override
    public ExecutionResult run(StepExecutionContext context) throws Exception {
        JSONObject origin = (JSONObject) context.getWorkflow().getData();
        JSONObject input = origin.optJSONObject(GenerateCloudInitSingleNode.class.getSimpleName());
        LOGGER.info("Input GenerateCloudInitSingleNode : " + input.toString());
        ...
        int portDefault = Integer.parseInt(origin.optJSONObject(CreateNetworkSingleNode.class.getSimpleName()).optString("portDefault"));
        ...
        try {
            ...
            if (agents != null && !agents.isEmpty()) {
                ...
                String configurationAgent = AgentManager.generateConfigurationAgent(instanceId, compute.getId(), datastore,
                        ...
                        dockerRegistry, backupUrl, backupMode, network.getIpAddress(), monitorResourceType);
                String configurationKafkaStand = DatastoreManager.generateConfigurationKafkaSingleNode(datastoreMode, portDefault, enableBasicAuth, authInfo);

                String configApplication = configurationAgent + configurationKafkaStand;
                ...
                LOGGER.warn("Not found agent by computeId = " + compute.getId());
            }
