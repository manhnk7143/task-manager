...
public class GenerateCloudInitCluster implements StepBody {
    ...
    public ExecutionResult run(StepExecutionContext context) throws Exception {
        ...
        int replicas = Integer.parseInt(input.optString("replicas"));
        ...
        try {
            ...
            for (int i = 0; i < masterNodes; i++) {
                ...
                if (agents != null) {
                    ...
                    if (i == 0) {
                        ...
                        configurationAgent = AgentManager.generateConfigurationAgent(instanceId, computeId, datastore,
                                datastoreVersion, agent.getEncryptedKey(), agent.getId(), Config.socket_url,
                                dockerRegistry, null, null, mapNetwork.get(computeId), monitorResourceType);

                    }
                    String configurationRedisCluster = DatastoreManager.generateConfigurationRedisCluster(datastoreMode,
                            Constaint.MASTER_ROLE, password, clusterNodes, portDefault, busPortRedis, replicas);

                    String configApplication = configurationAgent + configurationRedisCluster;
            ...
            for (int i = 0; i < slaveNodes; i++) {
                ...
                if (agents != null && !agents.isEmpty()) {
                    ...
                    String configurationRedisCluster = DatastoreManager.generateConfigurationRedisCluster(datastoreMode,
                            Constaint.SLAVE_ROLE, password, clusterNodes, portDefault, busPortRedis, replicas);
