...
public class TestCommandProcessor implements ProcessorBase<ControlJob> {

    private static final Logger LOGGER = Logger.getLogger(TestCommandProcessor.class);
    ...
    public boolean process(ControlJob job) throws Exception {
        ...
        LOGGER.info("[4CMS] TestCommandProcessor " + job.getServiceId() + " - " + job.getData());
        job.decodePacket();
        ...
        if (account != null) {
            LOGGER.info("Send message to agentId " + agentId + ", " + account.getClients().size());
            ResponsePacket responsePacket = new ResponsePacket();
            responsePacket.setServiceId(AppServiceType.TEST_COMMAND);
