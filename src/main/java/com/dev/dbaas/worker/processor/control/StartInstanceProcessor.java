...
public class StartInstanceProcessor implements ProcessorBase<ControlJob> {

    private static final Logger LOGGER = Logger.getLogger(StartInstanceProcessor.class);
    ...
    public boolean process(ControlJob job) throws Exception {
        ...
        LOGGER.info("[4CMS] StartInstanceProcessor " + job.getServiceId()+" - "+job.getData());
        job.decodePacket();
        ...
        for(TbAgent agent : agents){
            ...
            if(account != null){
                ...
                responsePacket.setServiceId(AppServiceType.START_INSTANCE);
