...
public class SetPasswordInstanceProcessor implements ProcessorBase<ControlJob> {

    private static final Logger LOGGER = Logger.getLogger(SetPasswordInstanceProcessor.class);
    ...
    public boolean process(ControlJob job) throws Exception {
        ...
        LOGGER.info("[4CMS] SetPasswordInstanceProcessor" + job.getServiceId()+" - "+job.getData());
        job.decodePacket();
        ...
        for(TbAgent agent : agents){
            ...
            if(account != null){
                ...
                responsePacket.setServiceId(AppServiceType.SET_PASSWORD);
