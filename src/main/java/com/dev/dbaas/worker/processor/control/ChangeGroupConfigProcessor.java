...
public class ChangeGroupConfigProcessor implements ProcessorBase<ControlJob> {

    private static final Logger LOGGER = Logger.getLogger(ChangeGroupConfigProcessor.class);
    ...
    public boolean process(ControlJob job) throws Exception {
        ...
        LOGGER.info("[4CMS] ChangeGroupConfigProcessor" + job.getServiceId()+" - "+job.getData());
        job.decodePacket();
        ...
        for(TbAgent agent : agents){
            ...
            if(account != null){
                ...
                dataJson.put("overridesConfig", overridesConfig);
                dataJson.put("agentId", agent.getId());
                dataJson.put("groupConfigId", groupConfigId);
                LOGGER.info("Packet "+instanceId+" "+overridesConfig);

                ResponsePacket responsePacket = new ResponsePacket();
                responsePacket.setServiceId(AppServiceType.CHANGE_GROUP_CONFIG);
