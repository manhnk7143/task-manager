...
public class RestartInstanceProcessor implements ProcessorBase<ControlJob> {

    private static final Logger LOGGER = Logger.getLogger(RestartInstanceProcessor.class);
    ...
    public boolean process(ControlJob job) throws Exception {
        ...
        LOGGER.info("[4CMS] RestartInstanceProcessor " + job.getServiceId()+" - "+job.getData());
        job.decodePacket();
        ...
        for(TbAgent agent : agents){
            ...
            if(account != null){
                ...
                responsePacket.setServiceId(AppServiceType.RESTART_INSTANCE);
