...
public class InitConfigParam {
    ...
    public void init() {
        ...
        DatastoreModeEntity datastoreModeMasterSlave = datastoreModeRepository.findFirstByNameAndDatastoreVersionId(
                "Cluster", datastoreVersion.getId()).orElse(new DatastoreModeEntity());
        if (datastoreModeMasterSlave.getId() == null) {
            datastoreModeMasterSlave.setName("Cluster");
    ...
    private void initConfigParams(String jsonConfig, String datastoreModeId) {
        ...
        Config lstConfig = objectMapper.readValue(jsonConfig, Config.class);
    ...
    @GetMapping("/clear-data-openstack-database")
    public void clearDatabaseAndDevStack() {
        ...
        List<ComputeEntity> lstCompute = computeRepository.findAll();
        for(ComputeEntity computeEntity : lstCompute) {
            if (computeEntity.getNovaInstanceId() != null) {
                Server server = os.compute().servers().get(computeEntity.getNovaInstanceId());
                if (server != null) {
                    os.compute().servers().delete(computeEntity.getNovaInstanceId());
                }
            }
        }
        computeRepository.deleteAll();

        List<NetworkEntity> lstNetwork = networkRepository.findAll();
        Set<String> sgIds = new LinkedHashSet<>();
        for(NetworkEntity networkEntity : lstNetwork) {
            String portId = networkEntity.getNeutronPortId();
            if (portId != null) {
            ...
            if ("user".equals(networkEntity.getMode())) {
                if (networkEntity.getNeutronSgId() != null) {
                    sgIds.add(networkEntity.getNeutronSgId());
                }
            }
        }

        for (String sgId : sgIds) {
            SecurityGroup sg = os.networking().securitygroup().get(sgId);
            if (sg != null) {
                os.networking().securitygroup().delete(sgId);
            }
        }
